...
// הוספת אפקט נצנוץ לכפתורי הקטגוריות
function addSparkleEffect() {
  const categoryCards = document.querySelectorAll('.category-card');
  
  categoryCards.forEach(card => {
    // הוספת 10 נקודות נוצצות לכל כפתור
    for (let i = 0; i < 10; i++) {
      ...
      sparkle.style.animation = `twinkle ${duration}s infinite ${delay}s`;
...
function addTwinkleAnimation() {
  ...
  try {
    ...
    console.error('שגיאה בהוספת אנימציית נצנוץ:', e);
  }
}

// הפעלת האפקטים בטעינת העמוד
document.addEventListener('DOMContentLoaded', function() {
  // קריאה לפונקציות קיימות
  initCube();
  initModal();
  initScrollEvents();
  initCategoryButtons();
